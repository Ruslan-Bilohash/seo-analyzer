# SEO Analyzer

**Мощний безкоштовний інструмент для аналізу SEO будь-якого сайту**

![SEO Analyzer](https://edukvam.com/seo/screen.jpg)

[![PHP](https://img.shields.io/badge/PHP-7.4%2B-777BB4?style=flat-square&logo=php&logoColor=white)](https://php.net)
[![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)](LICENSE)
[![Stars](https://img.shields.io/github/stars/Ruslan-Bilohash/seo-analyzer?style=flat-square)](https://github.com/Ruslan-Bilohash/seo-analyzer/stargazers)
[![Forks](https://img.shields.io/github/forks/Ruslan-Bilohash/seo-analyzer?style=flat-square)](https://github.com/Ruslan-Bilohash/seo-analyzer/network/members)

---

**Зроблено в Норвегії українцем з любов’ю ❤️**

---

### Live Demo
**[Відкрити демо →](https://edukvam.com/seo/index.php)**

### Підтримати автора
**[Купити каву автору →](https://edukvam.com/seo/donate.php)**

---

## English

**SEO Analyzer** is a powerful, fast and completely free online tool for comprehensive SEO analysis of any website.

It checks the most important on-page factors and provides clear, actionable recommendations to improve your site’s visibility in search engines.

### Key Features
- Full analysis of Title, Meta Description, Keywords, Robots and Open Graph
- H1 and H2 headings check
- Image alt text verification
- Real page loading time and content size measurement
- URL structure and readability analysis
- Internal, external and nofollow links statistics
- History of the last 20 analyses
- Subscription to new updates
- Order a full professional SEO audit
- Support for 8 interface languages
- Modern, clean and fully responsive design

### Screenshots
### Skjermbilder
![Skjermbilde 1](https://edukvam.com/seo/screen.jpg)
![Skjermbilde 2](https://edukvam.com/seo/screen2.jpg)
![Skjermbilde 3](https://edukvam.com/seo/screen3.jpg)

### Comparison Table

| Feature                        | SEO Analyzer | Google PageSpeed | SEMrush | Ahrefs | Screaming Frog |
|--------------------------------|--------------|------------------|---------|--------|----------------|
| Title & Meta Analysis          | Yes          | No               | Yes     | Yes    | Yes            |
| Alt text check                 | Yes          | No               | Yes     | Yes    | Yes            |
| Real load time                 | Yes          | Yes              | No      | No     | No             |
| URL structure                  | Yes          | No               | Yes     | Yes    | Yes            |
| Link analysis                  | Yes          | No               | Yes     | Yes    | Yes            |
| Analysis history               | Yes          | No               | No      | No     | No             |
| Free forever                   | Yes          | Limited          | Paid    | Paid   | Paid           |
| No registration                | Yes          | —                | Yes     | Yes    | Yes            |

---

## Українська

**SEO Analyzer** — сучасний, швидкий і **повністю безкоштовний** онлайн-інструмент для глибокого SEO-аналізу будь-якого сайту.

Він перевіряє найважливіші on-page параметри і видає чіткі рекомендації, які допоможуть покращити позиції сайту в пошукових системах.

### Основні можливості
- Повний аналіз Title, Meta Description, Keywords, Robots та Open Graph
- Перевірка H1 та H2 заголовків
- Перевірка alt-тегів у всіх зображеннях
- Реальний час завантаження сторінки + розмір контенту
- Аналіз структури та читабельності URL
- Статистика внутрішніх, зовнішніх та nofollow посилань
- Історія останніх 20 аналізів
- Підписка на оновлення інструменту
- Замовлення повного професійного SEO-аудиту
- Підтримка 8 мов інтерфейсу
- Сучасний адаптивний дизайн

### Скріншоти
![Скріншот 1](https://edukvam.com/seo/screen.jpg)
![Скріншот 2](https://edukvam.com/seo/screen2.jpg)

**Зроблено в Норвегії українцем з любов’ю ❤️**

**[Відкрити демо →](https://edukvam.com/seo/index.php)**  
**[Підтримати автора →](https://edukvam.com/seo/donate.php)**

---

## Norsk

**SEO Analyzer** er et kraftig, raskt og **helt gratis** online verktøy for omfattende SEO-analyse av ethvert nettsted.

Verktøyet sjekker de viktigste on-page faktorene og gir klare, handlingsrettede anbefalinger.

### Hovedfunksjoner
- Full analyse av Title, Meta Description, Keywords, Robots og Open Graph
- Sjekk av H1- og H2-overskrifter
- Alt-tekst kontroll på alle bilder
- Reell innlastingstid + innholdsstørrelse
- URL-struktur og lesbarhet
- Statistikk over interne, eksterne og nofollow-lenker
- Historikk over siste 20 analyser
- Abonnement på oppdateringer
- Bestilling av full profesjonell SEO-audit
- Støtte for 8 språk
- Moderne og responsivt design

**Laget i Norge av en ukrainer med kjærlighet ❤️**

**[Live Demo →](https://edukvam.com/seo/index.php)**  
**[Støtt prosjektet →](https://edukvam.com/seo/donate.php)**

---

### Автор
**Ruslan Bilohash**  
Email: rbilohash@gmail.com  
Telegram: [@meistru_lt](https://t.me/meistru_lt)  
GitHub: [Ruslan-Bilohash](https://github.com/Ruslan-Bilohash)
